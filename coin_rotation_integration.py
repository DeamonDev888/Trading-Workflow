...
from src.agents.persistent_agent_client import PersistentAgentClient, TradingTask, TaskPriority
...
class IntegratedRotationSystem:
        ...
        async with PersistentAgentClient() as client:
            ...
            while self.rotation_manager.running:
                try:
                    ...
                    if self.auto_optimization and (datetime.now() - self.last_optimization).seconds > 1800:
    ...
    async def _adjust_config_from_analysis(self, analysis: Dict[str, Any]):
        ...
        try:
            ...
            print(f"[CONFIG] Market: {market_condition}, Volatility: {volatility_level}, Liquidity: {liquidity_level}")
            ...
            if liquidity_level == "LOW":
                self.rotation_manager.config.max_assets = max(5, self.rotation_manager.config.max_assets - 3)
                print(f"[CONFIG] Reduced max assets to {self.rotation_manager.config.max_assets} due to low liquidity")
    ...
    async def _enhanced_initialization(self, analysis: Dict[str, Any]):
        ...
        try:
            ...
            if result and result.get("success"):
                ...
                print(f"[INIT] Enhanced monitoring started for {len(self.rotation_manager.monitored_assets)} assets")
    ...
    def _calculate_enhanced_priority(self, asset_data: Dict[str, Any], analysis: Dict[str, Any]) -> int:
        ...
        try:
            ...
            # Composite priority
            priority = base_priority - int((1 - confidence) * 2) + liquidity_bonus + trend_bonus + market_boost
            ...
            try:
                ...
                task = TradingTask(
                    ...
                    payload={
                        ...
                        "monitoring_duration": (datetime.now() - profile.last_updated).total_seconds(),
                        "multi_timeframe": True,
                ...
                if result and result.get("success"):
                    ...
                    base_score = self.rotation_manager._calculate_performance_score(response, profile)
    ...
    async def _analyze_enhanced_rotation_needs(self,
        ...
        try:
            ...
            for rec in recommendations:
                ...
                if symbol and symbol not in current_symbols and symbol in recommended_symbols:
                    ...
                    composite_score = (
                        ...
                        recommendation_score * (1 - self.rotation_manager.config.performance_weight) +
                        market_score * 0.2
                    )
            ...
            for symbol, profile in self.rotation_manager.monitored_items():
                ...
                if (not symbol_in_recommendations and (poor_performance or adverse_market)) or current_score < 0.15:
                    decisions['assets_to_remove'].append({
                        'symbol': symbol,
                        'score': removal_score,
                        'performance_score': current_score,
                        'market_score': market_score,
                        'reason': self._get_removal_reason(poor_performance, adverse_market, current_score),
    ...
    def _get_removal_reason(self, poor_performance: bool, adverse_market: bool, score: float) -> str:
    ...
    async def _execute_optimized_rotation(self, decisions: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            available_slots = self.rotation_manager.config.max_assets - len(self.rotation_manager.monitored_assets)
    ...
    async def _execute_removal_with_validation(self, asset_info: Dict[str, Any], results: Dict[str, Any]):
    ...
    async def _execute_addition_with_validation(self, asset_info: Dict[str, Any], results: Dict[str, Dict[str, Any]]):
        ...
        if all(validation_results.values()):
            ...
            results['errors'].append(f"Asset {symbol} failed validation: {', '.join(failed_validations)}")

    async def _validate_asset_addition(self, symbol: str, asset_info: Dict[str, Any]) -> Dict[str, bool]:
    ...
    def _update_enhanced_metrics(self, cycle_time: float, results: Dict[str, Any]):
        ...
        if 'agent_tasks' in results:
            self.rotation_manager.performance_metrics['agent_tasks_used'] = \
                self.rotation_manager.performance_metrics.get('agent_tasks_used', 0) + len(results['agent_tasks'])

    async def _optimize_rotation_strategy(self):
        """Automatically optimize rotation strategy"""
        try:
            # Analyze recent performance
            recent_performance = self.rotation_manager.performance_metrics.get('successful_reallocations', 0)
            ...
            if total_rotations > 0:
                ...
                if success_rate > 0.8:
                    print(f"[OPTIMIZE] High success rate ({success_rate:.1%}), expanding asset universe")
                    self.rotation_manager.config.max_assets = min(20, self.rotation_manager.config.max_assets + 2)

                elif success_rate < 0.5:
                    print(f"[OPTIMIZE] Low success rate ({success_rate:.1%}), reducing asset universe")
                    self.rotation_manager.config.max_assets = max(5, self.rotation_manager.config.max_assets - 1)
                    self.rotation_manager.config.rotation_interval = min(600, self.rotation_manager.config.rotation_interval + 60)

                print(f"[OPTIMIZED] Updated max assets to {self.rotation_manager.config.max_assets}")
    ...
    async def _update_strategy_effectiveness(self):
        ...
        try:
            ...
            self.rotation_manager.performance_metrics['strategy_effectiveness'][current_strategy] = \
                self.rotation_manager.performance_metrics.get('strategy_effectiveness', {}).get(current_strategy, 0) + 1
...
if __name__ == "__main__":
    async def demo_integrated_rotation():
        ...
        print("[DEMO] Starting integrated rotation with workflow optimization (runs for 6 minutes)...")
