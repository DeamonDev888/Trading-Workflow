...
var log = {
    ...
    info: function (msg, category) {
        if (category === void 0) { category = 'SYSTEM'; }
        var timestamp = getTimestamp();
        console.log("[".concat(timestamp, "] [").concat(colors.cyan, "INFO").concat(colors.reset, "] [").concat(colors.blue).concat(category).concat(colors.reset, "] \u2139\uFE0F  ").concat(msg));
    },
    success: function (msg, category) {
        if (category === void 0) { category = 'SYSTEM'; }
        var timestamp = getTimestamp();
        console.log("[".concat(timestamp, "] [").concat(colors.green, "SUCCESS").concat(colors.reset, "] [").concat(colors.blue).concat(category).concat(colors.reset, "] \u2705 ").concat(msg));
    },
    error: function (msg, category) {
        if (category === void 0) { category = 'ERROR'; }
        var timestamp = getTimestamp();
        console.log("[".concat(timestamp, "] [").concat(colors.red, "ERROR").concat(colors.reset, "] [").concat(colors.magenta).concat(category).concat(colors.reset, "] \u274C ").concat(msg));
    },
    warn: function (msg, category) {
        if (category === void 0) { category = 'WARNING'; }
        var timestamp = getTimestamp();
        console.log("[".concat(timestamp, "] [").concat(colors.yellow, "WARN").concat(colors.reset, "] [").concat(colors.blue).concat(category).concat(colors.reset, "] \u26A0\uFE0F  ").concat(msg));
    },
    // 🚀 HyperLiquid Specific Logs
    hyperliquid: {
        api: function (msg) { return log.info(msg, 'HYPERLIQUID-API'); },
        price: function (symbol, price) {
            log.success("\uD83D\uDCB0 ".concat(symbol, ": $").concat(price.toLocaleString()), 'HYPERLIQUID-PRICE');
        },
        tokens: function (count) {
            log.success("\uD83D\uDCCA Loaded ".concat(count, " tokens from HyperLiquid API"), 'HYPERLIQUID-TOKENS');
        },
        error: function (msg) { return log.error(msg, 'HYPERLIQUID-ERROR'); },
    },
    // 📊 Trading Logs
    trading: {
        order: function (symbol, side, size) {
            log.info("\uD83D\uDCC8 ".concat(side.toUpperCase(), " ").concat(size, " ").concat(symbol), 'TRADING-ORDER');
        },
        position: function (symbol, action) {
            log.info("\uD83C\uDFAF Position ".concat(action, ": ").concat(symbol), 'TRADING-POSITION');
        },
        success: function (msg) { return log.success(msg, 'TRADING-SUCCESS'); },
        error: function (msg) { return log.error(msg, 'TRADING-ERROR'); },
    },
    // 💡 API Logs
    api: {
        ...
        response: function (path, status) {
            var color = status >= 200 && status < 300 ? colors.green : colors.red;
            console.log("[".concat(getTimestamp(), "] [").concat(color, "RESPONSE").concat(colors.reset, "] [").concat(colors.cyan, "API").concat(colors.reset, "] ").concat(path, " \u2192 ").concat(color).concat(status).concat(colors.reset));
        },
    ...
    agent: {
        start: function (type) {
            log.success("\uD83D\uDE80 Starting ".concat(type, " agent"), 'AGENT-CONTROL');
        },
        stop: function (type) {
            log.warn("\uD83D\uDED1 Stopping ".concat(type, " agent"), 'AGENT-CONTROL');
        },
        status: function (type, status) {
            log.info("\uD83D\uDCCA ".concat(type, " agent: ").concat(status), 'AGENT-STATUS');
        },
    },
    // 📈 Data Logs
    data: {
        load: function (source, count) {
            log.success("\uD83D\uDCE6 Loaded ".concat(count, " items from ").concat(source), 'DATA-LOAD');
        },
    ...
    perf: {
        start: function (label) {
            console.time("[".concat(getTimestamp(), "] [").concat(colors.magenta, "PERF").concat(colors.reset, "] ").concat(label));
        },
        end: function (label) {
            console.timeEnd("[".concat(getTimestamp(), "] [").concat(colors.magenta, "PERF").concat(colors.reset, "] ").concat(label));
        },
...
var HyperliquidAPI = null;
var HyperliquidWebSocket = null;
try {
    ...
    return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 3:
                    error_1 = _a.sent();
                    log.error("Failed to initialize HyperLiquid API: ".concat(error_1.message));
                    return [3 /*break*/, 4];
...
function initializeHyperLiquidWS() {
    try {
        ...
        log.error("Failed to initialize HyperLiquid WebSocket: ".concat(error.message));
    }
}
// ============================================================================
// WEBSOCKET SERVER
// ============================================================================
var wss = new ws_1.WebSocketServer({ port: WS_PORT });
wss.on('connection', function (ws) {
    ...
    ws.on('message', function (data) {
        try {
            ...
            log.info("WebSocket message received: ".concat(message.type), 'WEBSOCKET');
            ...
            switch (message.type) {
                ...
                case 'subscribe':
                    ...
                    break;
                default:
                    log.warn("Unknown WebSocket message type: ".concat(message.type), 'WEBSOCKET');
            }
        }
        catch (error) {
            log.error("WebSocket message parsing error: ".concat(error.message), 'WEBSOCKET');
        }
    });
...
function handleSubscription(ws, message) {
    var channel = message.channel, symbol = message.symbol;
    log.info("Subscription request: ".concat(channel, " for ").concat(symbol), 'WEBSOCKET');
...
app.get('/api/hyperliquid/tokens', function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var tokens, error_2;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 2, , 3]);
                if (!hlAPI) {
                    return [2 /*return*/, res.status(503).json({ error: 'HyperLiquid API not available' })];
                }
...
app.get('/api/hyperliquid/price/:symbol', function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var symbol, price, error_3;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 2, , 3]);
...
// Trading endpoints
app.post('/api/trading/order', function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var _a, symbol, side, size, price, result, error_4;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                _b.trys.push([0, 2, , 3]);
                _a = req.body, symbol = _a.symbol, side = _a.side, size = _a.size, price = _a.price;
                if (!hlAPI) {
                    return [2 /*return*/, res.status(503).json({ error: 'HyperLiquid API not available' })];
                }
                log.trading.order(symbol, side, size);
                return [4 /*yield*/, hlAPI.placeOrder({
                        symbol: symbol,
                        side: side,
                        size: size,
                        price: price
...
// Data endpoints
app.get('/api/data/backtest', function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var _a, symbol, strategy, backtestData;
    return __generator(this, function (_b) {
        try {
...
function executePythonScript(scriptPath, args) {
    ...
    return new Promise(function (resolve, reject) {
        var _a, _b;
        var pythonProcess = (0, child_process_1.spawn)('python', __spreadArray([scriptPath], args, true), {
        ...
        pythonProcess.on('close', function (code) {
            if (code === 0) {
                ...
                reject(new Error("Python script failed with code ".concat(code, ": ").concat(stderr)));
            }
        });
...
app.post('/api/python/execute', function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var _a, script, _b, args, result, error_5;
    return __generator(this, function (_c) {
        switch (_c.label) {
            case 0:
                _c.trys.push([0, 2, , 3]);
                _a = req.body, script = _a.script, _b = _a.args, args = _b === void 0 ? [] : _b;
                if (!script) {
                    return [2 /*return*/, res.status(400).json({ error: 'Script path is required' })];
                }
 ...
 */
app.get('/api/dashboard/real-time', function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var realData, error_6;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 2, , 3]);
            ...
            case 2:
                error_6 = _a.sent();
                log.error("Dashboard fetch error: ".concat(error_6.message), 'DASHBOARD-ERROR');
                res.status(500).json({
                    success: false,
...
function getRealTimeDataFromPythonAgents() {
    return __awaiter(this, void 0, void 0, function () {
        var spawn_1, path_2, hyperliquidData, riskData, fundingData, now, currentCycle, realData, error_7;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 4, , 5]);
                ...
                case 3:
                    fundingData = _a.sent();
                    now = new Date();
                    currentCycle = "".concat(now.getFullYear(), "-").concat((now.getMonth() + 1).toString().padStart(2, '0'), "-").concat(now.getDate().toString().padStart(2, '0'), "_").concat(now.getHours().toString().padStart(2, '0'), ":").concat(Math.floor(now.getMinutes() / 5) * 5);
                    realData = {
                        timestamp: new Date().toISOString(),
                        ...
                        alerts: __spreadArray(__spreadArray([], (riskData.alerts || []), true), (hyperliquidData.alerts || []), true),
                        agents_status: {
                ...
                case 4:
                    error_7 = _a.sent();
                    log.error("Error getting real data: ".concat(error_7.message), 'DASHBOARD-ERROR');
    ...
    return __awaiter(this, void 0, void 0, function () {
        return __generator(this, function (_a) {
            return [2 /*return*/, new Promise(function (resolve) {
                    var pythonScript = path_1.default.join(__dirname, '../src/algorithms/hyperliquid_agent.py');
                    var pythonProcess = (0, child_process_1.spawn)('python', [pythonScript, '--get-dashboard-data'], {
                        cwd: path_1.default.join(__dirname, '..'),
                        stdio: 'pipe',
                        env: __assign(__assign({}, process.env), { PYTHONPATH: path_1.default.join(__dirname, '..') })
                    });
                    var output = '';
                    ...
                    pythonProcess.stderr.on('data', function (data) {
                        log.error("HyperLiquid agent error: ".concat(data.toString()), 'HYPERLIQUID-AGENT');
                    });
                    pythonProcess.on('close', function (code) {
                        try {
                            if (output.trim()) {
...
function getRiskAgentRealData() {
    return __awaiter(this, void 0, void 0, function () {
        return __generator(this, function (_a) {
            return [2 /*return*/, new Promise(function (resolve) {
                    var pythonScript = path_1.default.join(__dirname, '../src/algorithms/risk_agent.py');
                    var pythonProcess = (0, child_process_1.spawn)('python', [pythonScript, '--get-dashboard-metrics'], {
                        cwd: path_1.default.join(__dirname, '..'),
                        stdio: 'pipe',
                        env: __assign(__assign({}, process.env), { PYTHONPATH: path_1.default.join(__dirname, '..') })
                    });
...
function getFundingAgentRealData() {
    return __awaiter(this, void 0, void 0, function () {
        return __generator(this, function (_a) {
            return [2 /*return*/, new Promise(function (resolve) {
                    var pythonScript = path_1.default.join(__dirname, '../src/algorithms/funding_agent.py');
                    var pythonProcess = (0, child_process_1.spawn)('python', [pythonScript, '--get-dashboard-summary'], {
                        cwd: path_1.default.join(__dirname, '..'),
                        stdio: 'pipe',
                        env: __assign(__assign({}, process.env), { PYTHONPATH: path_1.default.join(__dirname, '..') })
                    });
...
app.get('/api/dashboard', function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    ...
    return __generator(this, function (_a) {
        switch (_a.label) {
            ...
            case 2:
                error_8 = _a.sent();
                log.error("Dashboard error: ".concat(error_8.message), 'DASHBOARD-ERROR');
                res.status(500).json({
                    success: false,
 ...
 */
app.get('/api/bots', function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var botStatus, error_9;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 2, , 3]);
            ...
            case 2:
                error_9 = _a.sent();
                log.error("Bots status error: ".concat(error_9.message), 'BOTS-ERROR');
                res.status(500).json({
                    success: false,
...
function getBotStatusFromAgents() {
    return __awaiter(this, void 0, void 0, function () {
        var hyperliquidData, riskData, fundingData, error_10;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 4, , 5]);
                ...
                case 4:
                    error_10 = _a.sent();
                    log.error("Error getting bot status: ".concat(error_10.message), 'BOTS-ERROR');
                    return [2 /*return*/, {
                            total_bots: 4,
                            active_bots: 0,
                            bots: [],
...
function getSystemStatus() {
    return __awaiter(this, void 0, void 0, function () {
        var hyperliquidData, riskData, fundingData, error_12;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 4, , 5]);
...
function getTokensData() {
    return __awaiter(this, void 0, void 0, function () {
        var pythonScript_1, tokensData, error_14;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 2, , 3]);
                    pythonScript_1 = path_1.default.join(__dirname, '../src/algorithms/hyperliquid_agent.py');
                    return [4 /*yield*/, new Promise(function (resolve) {
                            var pythonProcess = (0, child_process_1.spawn)('python', [pythonScript_1, '--get-tokens'], {
                                cwd: path_1.default.join(__dirname, '..'),
                                stdio: 'pipe',
                                env: __assign(__assign({}, process.env), { PYTHONPATH: path_1.default.join(__dirname, '..') })
                            });
                ...
                case 2:
                    error_14 = _a.sent();
                    log.error("Error getting tokens: ".concat(error_14.message), 'TOKENS-ERROR');
 ...
 */
app.post('/api/agents/master/start', function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var masterAgentScript, env, masterProcess, outputBuffer_1;
    var _a, _b;
    return __generator(this, function (_c) {
        try {
            log.api.request('POST', '/api/agents/master/start');
            log.agent.start('MASTER_AGENT');
            masterAgentScript = path_1.default.join(__dirname, '../src/agents/master_agent.py');
            env = __assign(__assign({}, process.env), { PYTHONPATH: path_1.default.join(__dirname, '..') });
            masterProcess = (0, child_process_1.spawn)('python', [masterAgentScript], {
                detached: false,
                stdio: 'pipe',
                env: env
            });
            outputBuffer_1 = '';
            (_a = masterProcess.stdout) === null || _a === void 0 ? void 0 : _a.on('data', function (data) {
                outputBuffer_1 += data.toString();
                // Log agent output
                log.info("[MASTER_AGENT] ".concat(data.toString().trim()), 'AGENT-OUTPUT');
            });
            (_b = masterProcess.stderr) === null || _b === void 0 ? void 0 : _b.on('data', function (data) {
                log.error("[MASTER_AGENT ERROR] ".concat(data.toString().trim()), 'AGENT-ERROR');
            });
            masterProcess.on('error', function (error) {
                log.error("Master agent failed to start: ".concat(error.message), 'AGENT-ERROR');
            });
            masterProcess.on('exit', function (code) {
                if (code !== 0) {
                    log.error("Master agent exited with code ".concat(code), 'AGENT-EXIT');
                }
                else {
            ...
            log.error("Failed to start Agent Master: ".concat(error.message), 'AGENT-MASTER-ERROR');
            res.status(500).json({
                success: false,
                error: error.message,
                timestamp: new Date().toISOString()
 ...
 */
app.post('/api/agents/master/stop', function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var execSync;
    return __generator(this, function (_a) {
        try {
            ...
            try {
                ...
                log.warn("Could not stop Agent Master: ".concat(killError.message), 'AGENT-MASTER-WARNING');
            }
            res.json({
                success: true,
            ...
            log.error("Failed to stop Agent Master: ".concat(error.message), 'AGENT-MASTER-ERROR');
            res.status(500).json({
                success: false,
                error: error.message,
                timestamp: new Date().toISOString()
 ...
 */
app.get('/api/agents/master/status', function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var execSync, isRunning, pid, result, result, dashboardFile, hasDashboardData, dashboardData, lastUpdate;
    return __generator(this, function (_a) {
        try {
            ...
            try {
                if (process.platform === 'win32') {
                    result = execSync("powershell \"Get-Process | Where-Object {$_.ProcessName -like '*python*' -and $_.CommandLine -like '*master_agent*'} | Select-Object -First 1 -ExpandProperty Id\"", {
                        encoding: 'utf8',
                        stdio: ['pipe', 'pipe', 'pipe']
                    });
                    if (result.trim()) {
            ...
            dashboardFile = path_1.default.join(__dirname, '../backend/dashboard_data.json');
            hasDashboardData = fs_1.default.existsSync(dashboardFile);
            if (hasDashboardData) {
                dashboardData = JSON.parse(fs_1.default.readFileSync(dashboardFile, 'utf8'));
            ...
            log.error("Failed to get Agent Master status: ".concat(error.message), 'AGENT-MASTER-STATUS-ERROR');
            res.status(500).json({
                success: false,
                error: error.message,
                timestamp: new Date().toISOString()
 ...
 */
app.get('/api/backtests/validate', function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var backtesterScript, env, backtesterProcess_1, output_1, error_16, error_15;
    var _a, _b;
    return __generator(this, function (_c) {
        switch (_c.label) {
            case 0:
                ...
                backtesterScript = path_1.default.join(__dirname, '../src/data/realtime_backtester.py');
                env = __assign(__assign({}, process.env), { PYTHONPATH: path_1.default.join(__dirname, '..') });
                backtesterProcess_1 = (0, child_process_1.spawn)('python', [backtesterScript], {
                    cwd: path_1.default.join(__dirname, '..'),
                    stdio: ['pipe', 'pipe', 'pipe'],
                    env: env
                });
                output_1 = '';
                error_16 = '';
                (_a = backtesterProcess_1.stdout) === null || _a === void 0 ? void 0 : _a.on('data', function (data) {
                    output_1 += data.toString();
                    log.info("[BACKTESTER] ".concat(data.toString().trim()), 'BACKTESTER');
                });
                (_b = backtesterProcess_1.stderr) === null || _b === void 0 ? void 0 : _b.on('data', function (data) {
                    error_16 += data.toString();
                    log.error("[BACKTESTER ERROR] ".concat(data.toString().trim()), 'BACKTESTER-ERROR');
                });
                backtesterProcess_1.on('close', function (code) {
                    if (code === 0) {
                        ...
                        log.error("Backtest validation failed with code ".concat(code), 'BACKTESTER-ERROR');
                    }
                });
                // Wait for completion (with timeout)
                return [4 /*yield*/, new Promise(function (resolve, reject) {
                        backtesterProcess_1.on('close', resolve);
                        setTimeout(function () { return reject(new Error('Backtest validation timeout')); }, 30000);
                    })];
            case 1:
                // Wait for completion (with timeout)
                _c.sent();
            ...
            case 2:
                error_15 = _c.sent();
                log.perf.end('BACKTEST_VALIDATION');
                log.error("Backtest validation error: ".concat(error_15.message), 'BACKTEST-VALIDATION-ERROR');
            ...
            switch (_a.label) {
                ...
                case 1:
                    ...
                    app.listen(PORT, function () {
                        console.log('\n' + '='.repeat(80));
                        console.log("".concat(colors.green, "\uD83D\uDE80 NOVAQUOTE BACKEND SERVER STARTED").concat(colors.reset));
                        console.log('='.repeat(80));
                        console.log("".concat(colors.cyan, "\uD83D\uDCE1 HTTP Server:").concat(colors.reset, " http://localhost:").concat(PORT));
                        console.log("".concat(colors.cyan, "\uD83D\uDD0C WebSocket Server:").concat(colors.reset, " ws://localhost:").concat(WS_PORT));
                        console.log("".concat(colors.cyan, "\uD83D\uDD17 Health Check:").concat(colors.reset, " http://localhost:").concat(PORT, "/api/health"));
                        console.log('='.repeat(80) + '\n');
