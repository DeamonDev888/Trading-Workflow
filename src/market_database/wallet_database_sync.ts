/**
 * NOVAQUOTE Wallet Database Manager - better-sqlite3 Version
...
class WalletDatabase {
    ...
    createWallet(metamaskAddress: string, walletName: string, description?: string): number {
        ...
        const result = stmt.run(metamaskAddress, addressHash, walletName, description);
    ...
    getWalletByAddress(metamaskAddress: string): Wallet | null {
        ...
        return stmt.get(metamaskAddress) as Wallet || null;
    }
    ...
    getWalletById(walletId: number): Wallet | null {
        ...
        return stmt.get(walletId) as Wallet || null;
    }
    ...
    updateWalletVerification(walletId: number, isVerified: boolean, level: string): void {
        ...
        stmt.run(isVerified, level, walletId);
    }
    ...
    getCurrentBalance(walletId: number, currency: string = 'USD'): WalletBalance | null {
        ...
        return stmt.get(walletId, currency) as WalletBalance || null;
    }
    ...
    updateBalance(
        ...
        stmt.run(walletId, currency, availableBalance, totalBalance);
    }
    ...
    getBalanceHistory(walletId: number, days: number = 30): WalletBalance[] {
        ...
        return stmt.all(walletId) as WalletBalance[];
    }
    ...
    getTrades(walletId: number, limit: number = 50, offset: number = 0): Trade[] {
        ...
        return stmt.all(walletId, limit, offset) as Trade[];
    }
    ...
    upsertPosition(position: Omit<Position, 'id' | 'opened_at' | 'last_updated_at'>): number {
        ...
        const existing = existingStmt.get(position.wallet_id, position.symbol, position.exchange) as { id: number } | undefined;
    ...
    getActivePositions(walletId: number): Position[] {
        ...
        return stmt.all(walletId) as Position[];
    }
    ...
    closePosition(positionId: number, closePrice: number, finalPnl: number): void {
        ...
        stmt.run(closePrice, finalPnl, finalPnl, positionId);
    }
    ...
    getWalletSummary(walletId: number): WalletSummary | null {
        ...
        return stmt.get(walletId) as WalletSummary || null;
    }
