...
class ProvenStrategyLibrary:
    ...
    def validate_strategy_performance(
        ...
        if not is_valid:
            print(
                f"[WARNING] Stratégie {strategy_name} INVALIDÉE - Performance récente insuffisante"
            )
        else:
            print(
                f"[OK] Stratégie {strategy_name} validée - Performance: {recent_win_rate:.1%} win rate"
            )

        return is_valid

    def get_best_strategy_for_conditions(
        self, market_conditions: Dict, symbol: str
    ) -> Optional[Dict]:
        """
    ...
    def _calculate_condition_match(
        ...
        if strategy["category"] == "risk_management":
            ...
            if (
                market_conditions.get("sentiment_score", 0.5) > 0.75
                or market_conditions.get("sentiment_score", 0.5) < 0.25
            ):
    ...
    def add_new_strategy(self, strategy: Dict) -> bool:
        ...
        print(
            f"[OK] Nouvelle stratégie {strategy['name']} ajoutée - Win rate: {strategy['win_rate']:.1%}"
        )
        return True
