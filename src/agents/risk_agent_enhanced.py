...
class EnhancedRiskAgent:
    ...
    def comprehensive_analysis(self):
        ...
        if self.use_manager:
            ...
            agents = [
                ("bug-fixer", "Analyser et corriger tous les problèmes dans src/agents/risk_agent.py"),
                ("code-reviewer", "Audit de sécurité complet de risk_agent.py"),
                ("perf-optimizer", "Optimiser les performances des calculs de risque"),
                ("test-enhancer", "Créer une suite de tests complète pour risk_agent.py"),
                ("docs-generator", "Générer la documentation complète pour risk_agent.py")
            ]
            ...
            for agent_name, task in agents:
                ...
                cmd = [
                    "claude",
                    "--agents", f".claude/agents/novaquote-{agent_name}.json",
                    "--print",
    ...
    def quick_fix(self, issue_description):
        ...
        if self.use_manager:
            ...
            cmd = [
                "claude",
                "--agents", ".claude/agents/novaquote-bug-fixer.json",
                "--print",
                "--dangerously-skip-permissions",
    ...
    def validate_risk_logic(self):
        ...
        if self.use_manager:
            ...
            cmd = [
                "claude",
                "--agents", ".claude/agents/novaquote-code-reviewer.json",
                "--print",
                "--dangerously-skip-permissions",
                task
            ]
            try:
                subprocess_result = subprocess.run(cmd, capture_output=True, text=True, cwd=self.project_path)
                result = {
...
if __name__ == "__main__":
    ...
    if args.action == "analyze":
        ...
        print("Usage: python risk_agent_enhanced.py --action [analyze|fix|validate|quick] [--issue 'description'] [--file filepath]")