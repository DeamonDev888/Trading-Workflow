...
class HyperliquidAPI {
  ...
  async placeOrder(
    ...
    try {
      ...
      const orderData = this.signature.createOrder(
        asset,
        isBuy,
        price,
        size,
      ...
      const signedOrder = await this.signature.signOrder(
        this.privateKey,
        orderData
      );
  ...
  async placeMarketOrder(asset, isBuy, size, reduceOnly = false) {
    try {
      ...
      const orderData = this.signature.createMarketOrder(
        asset,
        isBuy,
        size,
        reduceOnly
  ...
  async cancelOrder(asset, orderId) {
    try {
      ...
      const signedCancel = await this.signature.signOrder(
        this.privateKey,
        cancelData
      );
  ...
  async modifyOrder(orderId, asset, isBuy, price, size, reduceOnly = false) {
    try {
      ...
      const modifyData = {
        type: 'modify',
        oid: orderId,
        order: this.signature.createModifyOrder(
          orderId,
          asset,
          isBuy,
          price,
      ...
      const signedModify = await this.signature.signOrder(
        this.privateKey,
        modifyData
      );
  ...
  async updateLeverage(asset, leverage, isCross = true) {
    try {
      ...
      const signedLeverage = await this.signature.signOrder(
        this.privateKey,
        leverageData
      );
  ...
  async transferUSDC(destination, amount, hyperliquidChain = 'Mainnet') {
    try {
      ...
      const signedTransfer = await this.signature.signUserAction(
        this.privateKey,
        transferData
      );
