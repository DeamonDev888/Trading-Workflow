...
class OpenAIModel(BaseModel):
    ...
    def initialize_client(self, **kwargs) -> None:
        ...
        try:
            ...
            safe_cprint(
                f"✨ Deamon Dev's magic initialized OpenAI model: {self.model_name} 🌟",
                "green",
            )
        ...
        try:
            if self.model_name.startswith(("gpt-5", "o1")):
                try:
                    ...
                    try:
                        ...
                        safe_cprint(
                            f"❌ Direct Responses HTTP fallback failed: {repr(http_e)}",
                            "red",
                        )
            ...
            if not content_text and hasattr(self.client, "responses"):
                try:
                    ...
                    safe_cprint(
                        f"❌ Deamon Dev Responses API fallback error: {repr(fallback_e)}",
                        "red",
                    )
            ...
            if (not content_text) and self.model_name == "gpt-5":
                try:
                    safe_cprint(
                        "🛟 Deamon Dev fallback: retrying with gpt-4o to avoid empty content",
                        "yellow",
                    )
                    fallback_messages = [
                        {"role": "system", "content": system_prompt},
                    ...
                    fb_response = self.client.chat.completions.create(
                        model="gpt-4o", messages=fallback_messages, **fb_kwargs
                    )
                    ...
                    if isinstance(fb_text, str) and fb_text.strip():
                        return ModelResponse(
                            ...
                            model_name="gpt-4o",
                            usage=(
                                fb_response.usage.model_dump()
                                if hasattr(fb_response, "usage")
                                else None
                            ),
                        )
                except Exception as fb_e:
                    safe_cprint(f"❌ Fallback to gpt-4o failed: {repr(fb_e)}", "red")
            ...
            try:
                ...
                if hasattr(e, "param"):
                    safe_cprint(f"🔎 param={getattr(e, 'param', None)}", "yellow")
