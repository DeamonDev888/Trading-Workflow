...
class OllamaModel(BaseModel):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if models:
                    ...
                    safe_cprint(f"📚 Available Ollama models: {model_names}", "cyan")
                    if self.model_name not in model_names:
                        safe_cprint(
                            f"⚠️ Model {self.model_name} not found! Please run:",
                            "yellow",
                        )
                        safe_cprint(f"   ollama pull {self.model_name}", "yellow")
                else:
                    safe_cprint("⚠️ No models found! Please pull the model:", "yellow")
                    safe_cprint(f"   ollama pull {self.model_name}", "yellow")
            else:
                safe_cprint(
    ...
    def generate_response(
        self, system_prompt, user_content, temperature=0.7, max_tokens=None, **kwargs
    ):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                safe_cprint(f"❌ Ollama API error: {response.status_code}", "red")
                safe_cprint(f"Response: {response.text}", "red")
    ...
    def get_model_parameters(self, model_name=None):
        ...
        try:
            ...
            safe_cprint(f"❌ Error getting model parameters: {str(e)}", "red")
            return None
